...
function Review({
  ...
  return (
    <div
      className={cn(
        "animate-fade-in hover:border-primary rounded-[2.25rem] border p-8 opacity-0 shadow-xl shadow-slate-900/5 transition-all duration-300",
        className,
      )}
      style={{ animationDelay }}
      {...props}
    >
      <p className="md:text-md text-pretty">{text}</p>
      <span className="text-muted-foreground mt-4 block">{name}</span>
