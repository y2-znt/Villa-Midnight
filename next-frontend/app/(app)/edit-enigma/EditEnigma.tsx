...
import { useEnigma, useUpdateEnigma } from "@/hooks/useEnigma";
...
export default function EditEnigma() {
  ...
  const { updateEnigma, isUpdating } = useUpdateEnigma();
  const { data: enigma } = useEnigma(id);
  ...
  const {
    ...
    defaultValues: {
      ...
      difficulty: enigma?.difficulty || "ONE",
      image: enigma?.image || null,
      numberOfParticipants: enigma?.numberOfParticipants || 1,
      numberOfHours: enigma?.numberOfHours || 1,
    },
  });
  ...
  useEffect(() => {
    if (enigma) {
      reset(enigma);
      setImagePreview(enigma.image);
    }
  }, [enigma, reset]);

  const onSubmit = async (data: EnigmaSchema) => {
    const token = getToken();
    if (!authUser?.user?.id || !id || !token) {
      toast.error("Vous devez être connecté pour modifier une énigme");
      return;
    }

    const enigmaData = { ...data, userId: authUser.user.id };
    updateEnigma({ id, data: enigmaData, token });
  };
